...
local HOTBAR_SLOTS_MINI: number = 5 -- Mobile gets 6 slots instead of default 3 it had before (changed to 5)
...
local function MakeSlot(parent: Instance, initIndex: number?): GuiObject
	...
	local BackgroundIcon: ImageLabel = nil -- Custom added by me (commu) to create smth similar to fisch
	...
	function slot:Fill(tool: Tool): ...any
		...
		local function assignToolData(): ()
			...
			if tool:HasTag("satchel") then
				BackgroundIcon.Image = "rbxassetid://104440673809536"
			elseif tool:HasTag("inventoryTool") then
				BackgroundIcon.Image = "rbxassetid://134993754727859"
	...
	BackgroundIcon = Instance.new("ImageLabel")
	BackgroundIcon.BackgroundTransparency = 1
	BackgroundIcon.Name = "BackgroundIcon"
	BackgroundIcon.Size = UDim2.fromScale(1, 1)
	BackgroundIcon.Position = UDim2.fromScale(0.5, 0.5)
	BackgroundIcon.AnchorPoint = Vector2.new(0.5, 0.5)
	if LEGACY_PADDING_ENABLED == true then
		BackgroundIcon.Size = UDim2.new(1, -SLOT_EQUIP_THICKNESS * 2, 1, -SLOT_EQUIP_THICKNESS * 2)
	else
		BackgroundIcon.Size = UDim2.fromScale(1, 1)
	end
	BackgroundIcon.ZIndex = 1
	BackgroundIcon.ImageTransparency = 0.75
	BackgroundIcon.Parent = SlotFrame
